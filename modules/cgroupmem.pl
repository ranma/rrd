#!/usr/bin/perl
# $Id: cgroupmem.pl,v 1.17 2007/04/04 22:02:20 mitch Exp $
#
...
# set variables
my $datafile = "$conf{DBPATH}/cgroupmem.rrd";
my $picbase  = "$conf{OUTPATH}/cgroupmem-";
...
for my $idx ( 0..19 ) {
    ...
    my $fname = "/cgroup/$path[$idx]/memory.usage_in_bytes";
    if ($path[$idx] eq "default") {
    	$fname = "/cgroup/memory.usage_in_bytes";
    }
    if (-e $fname && $path[$idx] ne "") {
	open MEM, $fname or die "can't open $fname: $!";
	$size[$idx] = <MEM>;
	chomp $size[$idx];
...
foreach ( [3600, "hour"], [86400, "day"], [604800, "week"], [31536000, "year"] ) {
    ...
    RRDs::graph($picbase . $scale . ".png",
		...
		'--lazy',
		'--imgformat=PNG',
		"--title=${hostname} cgroup rss+cache usage (last $scale)",
		'--upper-limit=100',
